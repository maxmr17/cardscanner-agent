// Social features: feed, posts, likes, comments, follows
...
// GET /social/feed — posts from followed users + self
router.get('/feed', authenticate, async (req, res) => {
  ...
  const { rows } = await db.query(
    `SELECT p.id, p.caption, p.created_at,
     ...
     GROUP BY p.id, u.id, ci.id, c.id, v.mid_price
...
// POST /social/posts — share a collection item
router.post('/posts', authenticate, async (req, res) => {
  ...
  const { rows } = await db.query(
    ...
    [req.user.id, collection_item_id, caption || null]
  );
