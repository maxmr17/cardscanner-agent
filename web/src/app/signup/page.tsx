  ...
  return (
    <div className="mt-2">
      <div className="flex gap-1">
        {[0,1,2,3].map(i => (
          <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${i < score ? colors[score - 1] : 'bg-gray-200 dark:bg-gray-700'}`} />
        ))}
      </div>
      <p className={`text-xs mt-1 ${score <= 1 ? 'text-red-500' : score === 2 ? 'text-yellow-500' : score === 3 ? 'text-blue-500' : 'text-green-500'}`}>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                id="email" type="email" required autoComplete="email" autoFocus value={email}
